# Concatenate models/*.smv into output/_main_model.smv. verify needs NUXMV.
# Host default: tools/nuxmv-linux (Linux) or tools/nuxmv-mac (Darwin).
...
OUTPUT_DIR ?= output
OUTPUT     ?= $(OUTPUT_DIR)/_main_model.smv
...
clean:
	rm -f $(OUTPUT) _main_model.smv tests/fixtures/minimal/_merged_test.smv
